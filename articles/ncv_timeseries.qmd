...
for(i in 1:11){
  ...
  # drop
  nei$a <- c(nei$a, iind)
  nei$ma<- c(nei$ma, length(iind))

  # want to predict forward 1 month
  iind <- which(mdays == (i+1))
  nei$d <- c(nei$d, iind)
  nei$md <- c(nei$md, length(iind))
}
nei$ma <- cumsum(nei$ma)
nei$md <- cumsum(nei$md)
```
...
nei <- list()
nei$d <- 1:365 #partygirl
nei$md <- 1:length(start)
...
- Thanks to Zachary Susswein for [spotting a bug](https://github.com/dill/calgary/issues/1) regarding a [change in the syntax](https://cran.r-project.org/web/packages/mgcv/ChangeLog) for `mgcv`. The `mgcvUtils` package has also been updated to take this into account.
