...
It's worth having a quick look at the structure that we're using to represent this in `mgcv`. First off, we note that we use `r length(b_ad$smooth[[1]]$S)` penalty matrices to fit the model:

```{r Smats}
length(b_ad$smooth[[1]]$S)
```

We can visualize these matrices simply as follows:

```{r}
#| label: visS
#| fig.cap: The penalty matrices used to fit our model `b_ad`. Note the spatial structure as we move left to right. Most values are zero but the leading diagonal advances down the matrix, showing where the penalty acts over the spline basis.
#| fig.width: 10
#| fig.height: 4
#| message: false
library(dplyr)

Splot <- lapply(b_ad$smooth[[1]]$S, array2DF, base=list(as.character(1:39)))
Splot <- do.call(rbind, Splot) %>%
  mutate(arow=as.numeric(Var1),
         acol=as.numeric(Var2),
         amat=rep(1:length(b_ad$smooth[[1]]$S),
                  each=nrow(b_ad$smooth[[1]]$S[[1]])*
                       nrow(b_ad$smooth[[1]]$S[[1]])),
         S=Value) %>%
  select(arow, acol, amat, S)


ggplot(Splot) +
  geom_tile(aes(x=acol, y=arow, fill=S)) +
  scale_y_reverse() +
  facet_wrap(~amat, nrow=1) +
...
# Bonus: plotting the smoothing parameter over the covariate

Reader Philip Dixon asks how to plot $\lambda(x)$ over $x$ to give an idea of how the smoothing parameter changes as a function of the covariate.

We start by noting that the penalty matrices for the adaptive smoother are also basis matrices for the spline that defines the smoothing parameter function $\lambda(x)=B \boldsymbol{\gamma}$. Since `mgcv` provides the penalties as a component of the returned object, we can simply extract these, multiply by the estimated coefficients, and sum to get the resulting function.

The function below is based on using the default values given to `s()` when for a univariate smoothing when using `bs="ad"`. Some fiddling may be required when using non-default options (for example, in some limited cases `m` and `bs`  will need to be checked/changed). `term` indicates which term in the model should be plotted.

```{r smooth-plot-sp}
#| label: smooth-plot-sp
#| fig.cap: Left plot shows the penalty as a function of time (actually over the interval the knots fall over, scaled to (0,1)), with the right plot showing the smoother itself to help understand the values.
#| fig.width: 10
#| fig.height: 4
#| message: false
...
The following plot shows this relationship alongside the smoother for comparison. What we can see here is a very small decrease from the starting value of the penalty (due to our increased uncertainty at the "edges" of the data), then a massive increase in the penalty around 30-40 ms. This increase coincides with a much larger range of data in the plot on the right. When the data is more varied like this, we need to increase the penalty value to ensure that the smoother does not try to fly off and visit all of the points.

```{r compare-smoopar-smoother}
par(mfrow=c(1,2))
plot_adaptive_1d(b_ad)
plot(b_ad, shift=coef(b_ad)[1])
points(mcycle$times, mcycle$accel)
```

The above function and a 2-dimensional version are available in the [`mgcvUtils`](https://github.com/dill/mgcvUtils) package.

# Thanks

Time to think about this was partly funded by [BioSS](https://www.bioss.ac.uk) "topic group": "what we talk about when we talk about random effects". Thanks also to John Addy for extremely useful discussions. Philip Dixon asked the interesting question about visualising the resulting smoothing function.
