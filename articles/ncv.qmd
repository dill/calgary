...
The leave-outs ($\alpha(k)$'s) are in the `a` element of the list. This is a vector of data indices in such an order that each of our cross-validation blocks is contiguous. If our first block is size `b` then we can get the block indices via `a[1]:a[b]`.
...
Next we need to set the *end points* of the blocks in the element `ma`, which in our case will be 100, 200, 300 and 400:

```{r}
nb$ma <- (1:4)*100
```

For the predictions ($\delta(k)$'s) we have a similar structure, `d` are the start points and `md` are the end points (these should be the same length).

Here let's try to predict only at the points we dropped each time. In that case `a == d`:
...
blerg <- lapply(1:4, function(x){
  ...
  rug(dat[[paste0("x", x-1)]][nb$a[1:nb$ma[1]]], col="red")
  rug(dat[[paste0("x", x-1)]][nb$a[(nb$ma[1]+1):nb$ma[2]]], col="green")
  rug(dat[[paste0("x", x-1)]][nb$a[(nb$ma[2]+1):nb$ma[3]]], col="blue")
  rug(dat[[paste0("x", x-1)]][nb$a[(nb$ma[3]+1):nb$ma[4]]], col="pink")
})
```
